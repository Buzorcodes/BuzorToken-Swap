...
pub contract BuzorToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @BuzorToken.Vault
        ...
        // Destroy the vault
        destroy() {
            BuzorToken.totalSupply = BuzorToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            BuzorToken.totalSupply = BuzorToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&BuzorToken.Minter>(/public/Minter, target: /storage/MinterStorage)
